...
namespace MultiFactor.Ldap.Adapter.Tests
{
    public class LdapStreamTests
    {
        ...
        [Theory]
        [InlineData(new byte[] { 0x30, 0x84, 0x7F, 0xFF, 0xFF, 0xFF })]
        [InlineData(new byte[] { 0x30, 0x84, 0xFF, 0xFF, 0xFF, 0xFF })]
        public async Task LdapStream_ShouldNotReadPacketWithOverflowingLength(byte[] header)
        {
            using (var stream = new MemoryStream(header))
            {
                ...
                Assert.Equal(header.Length, result.Data.Length);
                Assert.Equal(header, result.Data);
            }
        }
