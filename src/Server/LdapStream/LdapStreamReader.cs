...
namespace MultiFactor.Ldap.Adapter.Server.LdapStream
{
    public class LdapStreamReader 
    {
        ...
        public async Task<LdapPacketBuffer> ReadLdapPacket()
        {
            ...
            long declaredLength = (long)berLen.Length + berLen.BerByteCount + 1;
            if (berLen.Length < 0 || declaredLength > MAX_PACKET_SIZE)
            {
                _logger.Warning("LDAP packet of {size} byte(s) exceeds the {limit} byte(s) limit and will be bypassed without processing", declaredLength, MAX_PACKET_SIZE);
                return GetResultPacket(_readBuffer, totalRead, false);
            }
            int berLenWithHeading = (int)declaredLength;
